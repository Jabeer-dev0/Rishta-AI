...
const generateReport = async (req, res) => {
  try {
    ...
    // Cache in Match document if it exists
    await Match.findOneAndUpdate(
      { user: req.user._id, matchedUser: targetUserId },
      { $set: { ...report, report: report.reportText } },
      { upsert: false }
    );
...
const getStarInsight = async (req, res) => {
  try {
    ...
    await Match.findOneAndUpdate(
      { user: req.user._id, matchedUser: targetUserId },
      { $set: { 'aiInsights.starInsight': insight } },
      { upsert: false }
    );
...
const getCachedReport = async (req, res) => {
  try {
    const match = await Match.findOne({ _id: req.params.matchId, user: req.user._id })
      .populate('matchedUser', 'name age city profession interests personalityScores');
...
const getGuestSession = async (req, res) => {
  try {
    const session = await GuestCompatibility.findOne({ sessionId: req.params.sessionId });
